...
export const createMockServer: CreateMockServer = (config) => {
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      const { scan, watchDynamicFile, apiPrefix, mockDir } = serverConfig;
      // 获取项目根目录
...
async function useCustomServerMiddleware(
  ...
  try {
    if (req.headers.referer) {
      remote = new URL(req.headers?.referer).searchParams.get("remote") ?? "";
    }
    ...
    if (!allowExt.includes(fileExt as any)) {
      ...
      readStream.on("end", () => {
        logger.success(`✅ ReadStream End ${colorize(readPath, "underline")}`);
      });
      readStream.on("close", () => {
    ...
    if (err?.message.indexOf(mockNoEnabledStr) !== -1) {
      logger.info(`🔒 Mock Not Enabled! ${colorize(readPath, "underline")}`);
    } else if (
